...
const Products = () => {
  ...
  const HandelSubmit=async(e)=>{
   ...
   if(formdata._id){
    const res=await axios.put(`https://iphone-store-clone.onrender.com/api/updateproduct/${formdata._id}`,{
      name:formdata.name,
...
// console.log(res);
 ...
 setformdata({
      ...
      const createproduct = async () => {
        const res = await axios.post("https://iphone-store-clone.onrender.com/api/createproduct",{...formdata});
        // console.log(res.data);
  ...
  const handeldelete=async(_id)=>{
   ...
   const res= await axios.delete(`https://iphone-store-clone.onrender.com/api/deleteproduct/${_id}`);
   console.log(res);
  //  console.log(_id);
      ...
      <div className='container m-2 fs-3'>
        ...
        <form onSubmit={(e)=>HandelSubmit(e)} >
          ...
          <div className="row mb-3">
            ...
            <div className="col-sm-10">
              <input type="text" className="form-control form-control-lg " id="inputPassword3" maxLength="10" name="price" value={formdata.price} onChange={(e) => handleChange(e)} required />
            </div>
          </div>
            ...
            product.map((product, i) => {
              return <tbody  key={product._id}>
                ...
                <tr >
                  ...
                  <td>{product.price}</td>
                  <td>
                    <button className='m-1 btn btn-primary fs-4' onClick={(e)=>{HandelUpdate(product._id)}}>Edit</button>
                    <br></br>
                    <button className='btn btn-danger fs-4'onClick={(e)=>{handeldelete(product._id)}}>Delete</button>
                  </td>
                </tr>
